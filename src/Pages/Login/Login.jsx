...
const Login = () => {
    ...
    const hendelCapter =()=>{
        const value = valide.current.value
            ...
            <div className="relative h-[1100px] w-full overflow-hidden ">
                ...
                <div className="absolute bottom-0 flex h-3/4 w-full flex-col rounded-t-3xl bg-white bg-opacity-20 shadow">
                    <form onSubmit={hendelsubmit} className="mt-10 space-y-8 px-10 py-10 w-[500px] mx-auto text-center">
                        ...
                        <div className="group relative">
                            <input onBlur={hendelCapter} type="captcha" ref={valide} name="captcha" id="captcha" required className="peer h-14 w-full rounded-3xl bg-gray-100 px-4 text-sm outline-none" />
                            <label className="absolute left-2 top-0 flex h-full transform items-center pl-2 text-base transition-all duration-300 group-focus-within:-top-7 group-focus-within:h-1/2 group-focus-within:pl-0 group-focus-within:text-base group-focus-within:text-white peer-valid:-top-7 peer-valid:h-1/2 peer-valid:pl-0 peer-valid:text-base peer-valid:text-white">
                                D-
                            </label>
                        </div>
                        {/* <button onClick={hendelCapter} className="btn btn-active btn-neutral btn-xs ">Neutral</button> */}
                        <button disabled={desabled} type='submit' className="h-12 w-full rounded-3xl bg-blue-900 text-white transition-all duration-300 hover:bg-blue-800">Login</button>
